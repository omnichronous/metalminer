(function () {
    ...
    setlist.setlistfm = function (metaInfo, callback) {
        ...
        var uri = settings.corsProxy + 'http://api.setlist.fm/rest/0.1/search/artists.json?artistName=' + metaInfo.artist;
        uri = encodeURI(uri);
        request({ uri: uri }, function (err, res, body) {
            ...
            var uri = settings.corsProxy + 'http://api.setlist.fm/rest/0.1/artist/' + mbid + '/setlists.json';
            request({ uri: uri }, function (err, res, body) {
                try {
                    body = JSON.parse(body);
                } catch (e) {
