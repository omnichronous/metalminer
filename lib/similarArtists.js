(function () {
    ...
    sim.lastfm = function (metaInfo, callback) {
        ...
        var uri = settings.corsProxy + 'http://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist=';
    ...
    sim.pandora = function (metaInfo, callback) {
        ...
        var uri = settings.corsProxy + 'http://pandora.com/json/music/artist/';
        uri += metaInfo.artist.toLowerCase().replace(/ /gi, '-');
        request.get(uri, function (err, res, body) {
            if (err) {
                callback(undefined, { error: 'Request error' });
    ...
    sim.metalArchives = function (metaInfo, callback) {
        ...
        var uri = settings.corsProxy + 'http://www.metal-archives.com/search/ajax-advanced/searching/albums/?bandName=';
        ...
        request.get(uri, function (err, res, body) {
            ...
            if (body.aaData.length > 0 && body.aaData[0].length > 0) {
                ...
                var uri2 = settings.corsProxy + 'http://www.metal-archives.com/band/ajax-recommendations/id/';
                uri2 += match;
